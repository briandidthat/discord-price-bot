...
class SpotFetcher:
    ...
    def get_batch_spot_price(caller: str, symbols: list[str]) -> list[SpotPrice]:
        try:
            response = requests.post(f"{price_server_url}/spot/batch", json={"requests": symbols},
                                     headers={"caller": caller})
            data = response.json()
    ...
    def get_batch_historical_spot_price(caller: str, batch_request: BatchRequest) -> list[SpotPrice]:
        try:
            response = requests.post(f"{price_server_url}/spot/historical/batch", json=batch_request.serialize(),
                                     headers={"caller": caller})
            data = response.json()
            responses = [SpotPrice(x) for x in data]
            return responses
    ...
    def get_price_statistics(caller: str, symbol: str, start_date: str, end_date: str = None):
        ...
        request = Request(symbol, start_date, end_date)
        try:
            response = requests.post(f"{price_server_url}/spot/statistics", json=request.serialize(),
                                     headers={"caller": caller})
